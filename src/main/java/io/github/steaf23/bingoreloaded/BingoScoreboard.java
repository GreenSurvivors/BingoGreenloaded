...
public class BingoScoreboard {
    ...
    public BingoScoreboard(String worldName) {
        ...
        this.visualBoard = new InfoScoreboard(String.valueOf(ChatColor.ITALIC) + ChatColor.UNDERLINE + TranslationData.translate("menu.completed"), teamBoard);
    ...
    public void updateTeamScores() {
        ...
        BingoReloaded.scheduleTask(task ->
        {
            ...
            for (BingoTeam t : teamManager.getActiveTeams()) {
                if (t.card != null) {
                    objective.getScore(String.valueOf(t.getColor().chatColor)).setScore(t.card.getCompleteCount(t));
                }
            }
            updateVisual();
        });
    ...
    public void updateVisual() {
        ...
        for (BingoTeam team : teamManager.getActiveTeams()) {
            String teamScoreLine = ChatColor.DARK_RED + "[" + team.getColoredName().asLegacyString() + ChatColor.DARK_RED + "]" +
                    ChatColor.WHITE + ": " + ChatColor.BOLD + taskObjective.getScore(String.valueOf(team.getColor().chatColor)).getScore();
            visualBoard.setLineText(lineIndex, teamScoreLine);
            lineIndex += 1;

            if (!condensedDisplay) {
                for (BingoPlayer player : team.getPlayers()) {
                    String playerLine = String.valueOf(ChatColor.GRAY) + ChatColor.BOLD + " ┗ " + ChatColor.RESET + player.displayName();
