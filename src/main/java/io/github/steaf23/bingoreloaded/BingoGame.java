    ...
    public void startDeathMatch(int countdown) {
        if (countdown == 0) {
            ...
            for (BingoPlayer p : getTeamManager().getParticipants()) {
                ...
                p.gamePlayer().get().sendTitle(String.valueOf(ChatColor.GOLD) + ChatColor.GOLD + "GO", String.valueOf(ChatColor.DARK_PURPLE) + ChatColor.ITALIC + "find the item listed in the chat to win!", -1, -1, -1);
            }
            return;
        }
        ...
        for (BingoPlayer p : getTeamManager().getParticipants()) {
            ...
            p.gamePlayer().get().sendTitle(color + String.valueOf(countdown), "", -1, -1, -1);
            Message.sendDebug(color + String.valueOf(countdown), p.gamePlayer().get());
        }
